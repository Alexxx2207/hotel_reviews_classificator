...
from src.constants import PATHS, MLP_HIDDEN, MLP_DROPOUT
from src.features import load_vectorizer, transform
from src.baseline import load_baseline
from src.mlp import SparseMLP, csr_to_torch_sparse
...
def load_models():
    ...
    try:
        ...
        st.warning("Baseline model not found. Only MLP predictions will be available.")
        baseline_model = None
    ...
    if ckpt_path.exists():
        ...
        st.warning("MLP model checkpoint not found. Only baseline predictions will be available.")
    
    return vectorizer, baseline_model, mlp_model, mlp_device

def predict_baseline(vectorizer, model, text):
...
def predict_mlp(vectorizer, model, device, text):
    """Predict using MLP model"""
    if model is None:
        return None, None
    
    X = transform(vectorizer, [text])
    X_sparse = csr_to_torch_sparse(X, device)
...
# Main app
def main():
    st.markdown('<h1 class="main-header">Hotel Review Classifier</h1>', unsafe_allow_html=True)
    st.markdown("---")
    ...
    with col1:
        ...
        review_text = st.text_area(
            "Review Text",
            height=200,
            placeholder="Enter your hotel review here...\n\nExample: 'The hotel was amazing! Great service, clean rooms, and excellent location. Highly recommend!'",
        ...
        with example_col1:
            if st.button("Example Positive Review"):
                st.session_state.example_review = "This hotel exceeded all my expectations! The staff was incredibly friendly and helpful, the rooms were spotless and comfortable, and the breakfast buffet was outstanding. The location is perfect - close to all major attractions. I will definitely stay here again!"
        
        with example_col2:
            if st.button("Example Negative Review"):
                st.session_state.example_review = "Very disappointed with my stay. The room was dirty, the bed was uncomfortable, and the staff was unhelpful. The WiFi didn't work, and the noise from the street kept me awake all night. Would not recommend this hotel."
        
        # Use example review if set
        ...
        if st.button("Classify Review", type="primary", use_container_width=True):
            if not review_text.strip():
                ...
                if baseline_model is not None:
                    baseline_pred, baseline_probs = predict_baseline(vectorizer, baseline_model, review_text)
                
                if mlp_model is not None:
                    mlp_pred, mlp_probs = predict_mlp(vectorizer, mlp_model, mlp_device, review_text)
                ...
                if baseline_pred is not None:
                    ...
                    css_class = "positive" if baseline_pred == 1 else "negative"
                    st.markdown(
                ...
                if baseline_pred is not None and mlp_pred is not None:
                    ...
                    if baseline_pred == mlp_pred:
                        st.success(f"Both models agree: **{get_label_name(baseline_pred)}** review")
                    else:
                        st.warning(f"Models disagree: Baseline predicts **{get_label_name(baseline_pred)}**, MLP predicts **{get_label_name(mlp_pred)}**")
    
    # Footer
    st.markdown("---")
    st.markdown(
        "<div style='text-align: center; color: #666; padding: 2rem;'>"
        "Built with using Streamlit, PyTorch, and scikit-learn"
        "</div>",
        unsafe_allow_html=True
    )
