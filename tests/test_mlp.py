...
def test_mlp_forward_shape() -> None:
    """MLP forward returns logits with shape (batch_size, num_classes)."""
...
def test_iter_minibatches_yields_batches() -> None:
    ...
    reviews = np.arange(10).reshape(10, 1).astype(float)
    labels = np.arange(10)
    batches = list(iter_minibatches(reviews, labels, batch_size=3, shuffle=False))
...
def test_iter_minibatches_empty_batch_size_larger_than_data() -> None:
    ...
    batches = list(iter_minibatches(reviews, labels, batch_size=10, shuffle=False))
